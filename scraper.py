...
# La parade absolue contre les sites modernes (CBRE, JLL) : Playwright
# On force l'installation du navigateur Chrome invisible pour GitHub Actions
...
class RealPropertyScraper:
    ...
    def extract_price(self, text):
        """Récupère TOUS les prix. Si rien n'est trouvé, renvoie 0 (Nous consulter)"""
        text = text.replace('\xa0', '').replace(' ', '').replace('.', '')
        matches = re.findall(r'(\d+)€', text)
        if matches:
            prices = [int(m) for m in matches]
            return max(prices)
        return 0 # 0 sera interprété comme "Nous consulter" par le site web
    ...
    def scrape_real_estate_site(self):
        ...
        print("Lancement du radar (Mode Visuel Playwright activé pour vaincre le JS)...")

        # On ouvre le navigateur invisible
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=True)
            # On simule un vrai ordinateur
            ...
            for url in urls_a_visiter:
                ...
                try:
                    # On charge la page et on force une attente de 4 secondes pour que le JavaScript de CBRE/JLL s'affiche
                    ...
                    for annonce in annonces_html:
                        ...
                        # MODIFICATION : On garde l'annonce même si prix = 0
                        if surface == 0:
                            continue

                        titre_elem = annonce.find(['h2', 'h3', 'div', 'a', 'span'], class_=re.compile(r'title|name|heading|titre', re.IGNORECASE))
                        titre = titre_elem.text.strip() if titre_elem else f"Opportunité {nom_site.capitalize()}"
                        ...
                        prop_data = {
                            ...
                            "price": prix, # Peut être 0 (Nous consulter)
                            ...
                            "constructible": True,
                            "description": f"Extrait depuis {nom_site.upper()} avec le moteur Playwright.",
                    ...
                    print(f"   [!] Timeout ou erreur JS sur {nom_site} : {e}")

            browser.close() # On ferme le navigateur
    ...
    def calculate_score(self, prop):
        ...
        # Ajustement de la note si le prix est "Nous consulter"
        if prop['price'] == 0:
            score += 1.0 # Note neutre
        else:
    ...
    def run_pipeline(self):
        ...
        print(f"Annonces validant STRICTEMENT les 10 200m² : {len(unique_properties)}")

        if self.db_connected and unique_properties:
            print("Envoi vers la base de données Supabase...")
            try:
                self.supabase.table('properties').upsert(unique_properties).execute()
                print("✅ Succès ! Votre base de données a été mise à jour.")
            except Exception as e:
